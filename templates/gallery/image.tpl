				...
				<ul>
					{content}
						{if $image->largeThumbnailSize}
							<li><a href="{$image->getURL()}" title="{lang}gallery.image.viewFullSize{/lang}" class="button jsTooltip"><span class="icon icon16 fa-search-plus"></span> <span class="invisible">{lang}gallery.image.viewFullSize{/lang}</span></a></li>
						{/if}
...
>
	...
	<div class="section">
		<ul id="imageButtonContainer" class="galleryImageButtons buttonGroup jsImageInlineEditorContainer" data-object-id="{@$image->imageID}" data-is-deleted="{@$image->isDeleted}" data-is-disabled="{@$image->isDisabled}" data-can-delete-image="{@$image->canDelete()}" data-can-edit-image="{@$image->canEdit()}" data-can-moderate-image="{@$__wcf->session->getPermission('mod.gallery.canModerateImage')}">
			{if $image->canEdit()}
			...
			{/if}
			{if MODULE_LIKE && $__wcf->getUser()->userID && $__wcf->getSession()->getPermission('user.like.canViewLike') && $image->userID != $__wcf->user->userID}
