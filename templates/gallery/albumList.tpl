...
{capture assign='contentInteractionButtons'}
	{if $canAddAlbum}
		<a href="#" id="albumAddButton" class="contentInteractionButton button small jsOnly"><span class="icon icon16 fa-plus"></span> <span>{lang}gallery.album.add{/lang}</span></a>
	{/if}
...
{capture assign='contentInteractionDropdownItems'}
	{if !$feedControllerName|empty}
		<li><a rel="alternate" href="{if $__wcf->getUser()->userID}{link application='gallery' controller=$feedControllerName object=$controllerObject}at={@$__wcf->getUser()->userID}-{@$__wcf->getUser()->accessToken}{/link}{else}{link application='gallery' controller=$feedControllerName object=$controllerObject}{/link}{/if}" class="rssFeed">{lang}wcf.global.button.rss{/lang}</a></li>
	{/if}
{/capture}
